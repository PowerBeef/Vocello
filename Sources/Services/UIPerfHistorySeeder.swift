...
/// History seeding for the `ui_test.sh macos perf` lane.
///
...
/// share one 0.5 s silent WAV under `outputs/uiperf/`, and `createdAt` is
/// spread backwards over days so History's date sections and sorts are
/// genuinely exercised.
enum UIPerfHistorySeeder {
    ...
    /// A minimal valid 24 kHz mono PCM16 WAV of 0.5 s silence, written once.
    private static func writeSilentWAV() throws -> String {
        ...
        let frames = UInt32(sampleRate / 2)
