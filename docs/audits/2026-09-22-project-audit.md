...
## Verification addendum (main `0beb6331`, September 22)

Every finding ID was re-checked against source at `0beb6331` and live GitHub state before this
report landed on `main`. The report holds overall: no finding is invented, and where it is off it
overstates impact or cites stale lines. Remediation is tracked in plan `project-audit-2026-09` in
[`config/roadmap.json`](../../config/roadmap.json); the takeover audit
[`project-audit-2026-09-22.md`](../reference/project-audit-2026-09-22.md) is the plan's authority and
links here.

**Already fixed on `main`:**
- CI-06, the TSan lane base (`b36c1418`).
- CI-13 and most of DOC-02, the stale Claude-era references (`91e521ff`, `015951c4`).
- CI-20, mostly: `.claude/settings.json` asks before the XcodeBuildMCP device and test tools.
- Issue #86, fixed in source by `f6404cec`.

**Worse than stated:**
- **CI-01** has three stacked blockers, not one:
  - the release jobs install numpy but not pytest/pytest-xdist;
  - the `package` job also runs `--installed release`, whose `asc` only `archive-ios` installs;
  - the runner image now ships gh 2.100.0 against the exact 2.96.0 pin.
- **ENG-01** also affects the speech-tokenizer *encoder*, which ignores its 250-frame window
  (`MLXAudioCodecs/Mimi/Transformer.swift:158-164`). That covers clone references longer than 10 s.
- **IOS-01:** `beginBackgroundTask` appears nowhere in `Sources`, and the `.active` handler never
  clears a deferred background release (new, IOS-02 in the roadmap item).

**Overstated or wrong (corrected severity in parentheses):**

| Finding | Correction |
| --- | --- |
| ENG-03 (Low) | Deletes only the rebuildable `.qvoice_prepared_model` overlay, never the install |
| ENG-01 memory figures | A take is capped at 2,048 frames, so KV is about 128 MiB; the divergence claim stands |
| ENG-04, ENG-05 (Low) | Host digest pinning makes most triggers unreachable. ENG-05's missing MLX error handler matters for CORE-03 |
| ENG-16 | No `max_tokens` mis-mapping |
| CORE-06 | The idle-unload half is already guarded; the trim-versus-prefetch race is real |
| CORE-08 (Low-Med) | Only legacy or repairable installs reach it |
| CORE-20 | `AnyTTSEngineBackend` is live |
| TEST-01 | The 15% figure omits the linked and tested `QwenVoiceCore` and counts view code. The real gap is the orchestrators (`TTSEngineStore`, `StudioGenerationCoordinator`, the long-form runner, `DatabaseService`, `ModelManagerViewModel`, `IOSExportGate`) |
| TEST-02 | The decoder and code predictor already run in CI on tiny random weights; the talker and generate loop do not |
| TEST-03 (Medium) | Marking plumbing is tested and fails closed |
| TEST-16, TEST-17 | Coverage exists as an opt-in; the "worker" tests are child-process entry points |
| MAC-02 | 8 GB Macs already default to Speed; the impact is on 16 GB and larger Macs |
| MAC-04 (Low) | Rare packaging-failure path |
| MAC-22 | Language detection is already debounced |
| IOS-03, IOS-08, IOS-11 | Details overstated: the category flip happens on return to active; Wi-Fi-only is deliberate policy and only the copy is missing; one lock is non-blocking |
| SEC-01 (Medium) | Single admin collaborator; the threat is a stolen maintainer credential |
| SEC-09 (Info) | Bundled resource first, cwd only as a fallback |
| CI-04 | Dependabot PR #100 did receive a CI run |
| DOC-05 | The files live under `docs/decisions/`, and one of the three is not stale |
| Section 13 | Roadmap counts predate plan `project-audit-2026-09`: 71 open items |

**Report findings already on the roadmap:**

| Report ID | Roadmap item |
| --- | --- |
| CORE-01 | PA-01 |
| SEC-06 | PA-02 |
| IOS-15 | PA-03 |
| CI-01 (part) | PA-05 |
| SEC-12, ENG-18 (part) | PA-07 |
| ENG-14, MAC-23 (part) | PA-08 |
| IOS-05, CORE-09 | F-25 (widened) |
| SEC-14 | ASR-02 |
| SEC-03, SEC-10 | ASR-04 |
| CORE-05 | ASR-06 |
| TEST-05 | AV-13 |
| AUD-01 to AUD-12 | unchanged |

The remaining findings are grouped as PA-09 to PA-26, and PA-05 widens to the whole release path. The ENG-02 fix intentionally changes some
quality-first output: takes that were truncated get their tail back, and ICL clone takes may shift
by one sample.
