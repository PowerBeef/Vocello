...
<p align="center">
  ...
  <a href="https://github.com/PowerBeef/Vocello/releases/tag/v2.4.0"><img src="https://img.shields.io/badge/Vocello-2.4.0-7b61ff?style=flat-square" alt="Vocello 2.4.0"></a>
...
<p align="center">
  <a href="https://github.com/PowerBeef/Vocello/releases/download/v2.4.0/Vocello-macos26.dmg"><strong>Download Vocello 2.4.0 for macOS 26+</strong></a><br>
  <a href="https://testflight.apple.com/join/Cvp6yCv7"><strong>Join the iPhone beta on TestFlight</strong></a><br>
  <a href="https://github.com/PowerBeef/Vocello/releases/tag/v2.4.0">Release details</a> · <a href="docs/releases/v2.4.0.md">What is new</a> · <a href="https://github.com/PowerBeef/Vocello/releases">All releases</a>
</p>

<p align="center">
  <strong>Which build this page describes:</strong> the Mac download is Vocello 2.4.0, while this page follows the current source, the upcoming Vocello 3.0. The Mac screenshots show the 3.0 interface; the iPhone screenshot is an earlier capture whose mode tab still reads Custom. Features new in 3.0 are marked <strong>(3.0)</strong>.
</p>
...
| Describe character, age, accent, texture, and delivery. Save a successful design as a reusable voice reference. | Record in the app or import a file: WAV, MP3, AIFF, M4A, or FLAC on the Mac, plus OGG where macOS can decode it; WAV, MP3, AIFF, or M4A from Files on iPhone. A transcript improves conditioning but is optional. Generation requires the visible consent acknowledgment in Settings; only clone voices you own or are authorized to use. |
...
4. Generate from Custom Voice (Built-in Voice in 3.0), Voice Design, or Voice Cloning.
...
Upgrading from an earlier Vocello 2.x does not require reinstalling models; application data remains under `~/Library/Application Support/QwenVoice/`. After updating to 2.3.0 or newer, Settings shows an optional in-place update for installed models to the current smaller packages.
...
Every number below comes from a tracked, privacy-safe benchmark record in this repository and was measured on the support-floor tier, a Mac mini M2 with 8 GB, the canonical benchmark host until 2026-09-22. The chart below pools the takes of the most recent canonical records that share one comparison lineage and one app build (up to five), so one run's noise does not move a bar; today that pool is record `379db820` alone, measured with a Vocello 3.0 build from source. Canonical records now come from a Mac mini M6 with 16 GB and start a new series; M2 and M6 records are never compared.
...
<picture>
  ...
  <img alt="Grouped bar chart: warm real-time factor of the Speed models by script length, lower is faster: Built-in Voice 0.58 to 0.69, Voice Design 0.53 to 0.71, Voice Cloning 0.55 to 0.82. Every bar sits below the real-time line at 1.0." src="docs/charts/rtf-by-mode-light.svg">
</picture>
<!-- rtf-chart:end -->
...
ordinary macOS deterministic lane. Xcode 26 (the pinned CI toolchain) does not support executing the standalone tool-hosted,
...
> **Reproducing the benchmark numbers:** `./scripts/build.sh cli` produces an unoptimized development build that lands around a real-time factor of 1.0 (generation as long as the audio). The published figures come from the optimized Release path, and the two build topologies are not comparable; see the like-for-like rules in [`docs/reference/benchmarking-procedure.md`](docs/reference/benchmarking-procedure.md).
