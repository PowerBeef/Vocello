...
| 9 — Runtime component reuse | **Closed 2026-07-26.** Speech-tokenizer residency ships on macOS: a model switch adopts the previous model's tokenizer object when the host-attested content identity (trusted-checkpoint weights + config digests, hard-link inode fallback) matches, with an encoder-superset capability rule. Memory relief and explicit unload clear the slot (`Qwen3TTSMemoryCaches.clearAll`); only in-process switches preserve it; iOS stays disabled (its switch path pre-clears for Jetsam headroom). Evidence: fixed-seed switch A/B byte-identical with residency on/off, adoption probe `speech_tokenizer_load` 503→0 ms with ~260 ms better cold first-chunk latency, retained-memory qualification PASS on the 8 GB floor (`mac-memory-qualification-20260726-115343-5a1c8a85`, Custom→Design→Clone ×3). Gated off-switch: `QWENVOICE_TOKENIZER_RESIDENCY`. |
...
| 12 — Bounded analysis and unified quality | **Fast-depth registry shipping 2026-07-26.** Every shipping finalization emits a typed `GenerationQualityReport` (five fast gates), folds the persisted-WAV Fast QC verdict into the typed `persistedWAV` gate with its analyzer version and published-file digest, evaluates it through the fail-closed `QualityGateRegistry`, and records the verdict + resource-schedule stages in the open telemetry maps (no schema change). Live-verified (`quality_registry_outcome=pass` on a real take). 2026-07-26 additions: the standard/canonical depth producer (`deepReport` + typed `DeepGateEvidence`, fail-closed on missing analyzer evidence), per-take prosody gate verdicts on the bench delivery sidecar, and the advisory speaker-similarity dev-lane metric (`scripts/clone_speaker_similarity.py`). Open: lane emission of standard/canonical reports (bench/lang-bench call sites; long-form segment-verdict threading), optional MOS-proxy advisory. |
| 13 — Benchmark/history v3 | **Implemented 2026-07-26; first live v3 record pending.** `benchmarks/schema-v3.json` adds the typed quality-registry identity to every generation take (`qualityRegistryOutcome` pass/warning only — fail never publishes; `qualityRegistryRequiredGates` must cover the five fast gates, sorted; `qualityRegistryIssues` machine codes, required for a warning verdict, forbidden on a pass). The validator gains version-gated key sets (v1/v2 reject the new keys, preserving immutability), and the publisher folds the identity from the engine rows' phase-12 telemetry notes, stamping v3 when every take carries it, v2 when none does, and refusing a mixed record. v1/v2 records remain valid history; the first bench run on this tree publishes v3. |
...
3. **Stage 2 — memory program (folds phase 9)** — in progress 2026-07-26
   ...
   **Stage 2 is complete, including the promotion**: on 2026-07-26 the maintainer approved
   promoting 2.2 alone, all six production artifacts were converted with the pinned
   deterministic tooling (`scripts/convert_text_embedding_8bit.py`) and uploaded to public
   `PowerBeef02/<folder>` Hugging Face repos, and the contract/iOS-catalog/receipts were
   re-pinned as artifactVersion **2026.07.26.1** with `validate --require-complete` PASS
   (§N promotion entry). An isolated Mac delivery proof downloaded `pro_custom_speed`
   end-to-end from the new pins through the fail-closed path. Because installed models
...
4. **Stage 3 — quality harness (folds phase 12, then 13)** — in progress 2026-07-26,
   ...
   `GenerationQualityReportProducer.deepReport` composes the five fast gates with typed
   `DeepGateEvidence` per required deeper gate and maps missing analyzer evidence to
   `.unavailable` (registry fails closed). ~~Per-take prosody verdicts~~ **shipped** —
   `prosody_quality_gate.evaluate_metrics` verdicts now ride each `bench-prosody.json`
   delivery row from the same analysis pass. ~~Speaker-similarity dev-lane metric~~
   **shipped** — `scripts/clone_speaker_similarity.py` (revision-pinned SpeechBrain ECAPA
   backend behind an injectable boundary, advisory bands, offline fixture suite in the
   self-test gate; never a CI/packaging gate). ~~Typed languageASR gate~~ **shipped** on
