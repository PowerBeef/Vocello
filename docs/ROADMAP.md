...
| `delivery-prompting-2026-08` | active | backend-mlx | 27/33 (82%) |
...
| `DP-30` | planned | Run prompt, sampling, instruction-language and script-interaction screens | `file:scripts/tests/test_delivery_prompt_remediation.py`, `doc:docs/reference/delivery-harness.md` |
...
- **`DP-30`** (planned) — Run prompt, sampling, instruction-language and script-interaction screens.
  gate: The source-bound runner supports named subsets, exact receipts, retained failures, paired acoustics and bounded temporal contours. Exploratory global/minimal, sampling, and Happy/Surprised screens remain as previously recorded. A maintainer-directed 2026-08-24 production-copy checkpoint separately replaced both tiers of Happy, Angry, Fearful, and Surprised without claiming measured improvement; its source-bound Aiden smoke passed receipt/QC integrity only. PRE-REGISTERED 2026-08-25 before new generation: config/delivery-prompt-remediation-contract.json freezes six exact candidate strings and digests (Happy acoustic and emotion+acoustic; Angry emotion+acoustic; Fearful structured and conflict-reduced urgent; Surprised onset/settle), their order, target/competing preset, feature directions/weights, result vocabulary, stable controls, and four disjoint stages. Screen uses development seeds 32060826...32060829, Aiden/Ryan English plus Vivian Chinese and Sohee Korean, neutral medium scripts, Speed and official sampling. Script interaction uses 32060830...32060833 over short/medium/long neutral/congruent/conflicting text. Powered development uses 32060840...32060847 over all nine native cells plus four fixed sentinels. Untouched variant confirmation uses 33060826...33060833 with Speed and Quality kept separate. The composer reuses digest-verified neutral controls without another model launch, keeps failures in the denominator, makes Surprised temporal-first and Fearful tremor/urgency-aware, measures competing-preset distance, and fails closed on missing features or subgroup regressions. Automatic results can only be automatic_acoustic_improvement, no_measured_improvement, regression, inconclusive, or abstained_out_of_distribution; they have no semantic, production-copy, or publication authority. DP-31/DP-32 remain the human semantic promotion boundary.
