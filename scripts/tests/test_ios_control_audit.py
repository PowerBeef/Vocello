...
class IOSControlAuditContractTests(unittest.TestCase):
    ...
    def test_settings_reveal_requires_complete_dock_clearance(self) -> None:
        ...
        self.assertIn("search.nextScroll(target: required, visible: visible)", reveal)
        self.assertIn("VocelloUITouchScrollAnchor.index(", reveal)
        self.assertIn("container.staticTexts.allElementsBoundByIndex", reveal)
        self.assertIn("anchor.swipeUp(velocity: .slow)", reveal)
        self.assertIn("anchor.swipeDown(velocity: .slow)", reveal)
        self.assertNotIn("scroll(byDeltaX:", reveal)
        self.assertIn("containers.count == 1", reveal)
        self.assertIn("visible.contains(CGPoint(x: container.frame.midX, y: container.frame.midY))", reveal)
        self.assertIn("container.descendants(matching: target.elementType)", reveal)
        self.assertIn("!target.identifier.isEmpty && matches.count == 1", reveal)
        self.assertIn("matches.element.frame == frame", reveal)
        self.assertNotIn("app.swipeUp()", reveal)
        self.assertNotIn("app.swipeDown()", reveal)
        self.assertNotIn("coordinate(", reveal)
