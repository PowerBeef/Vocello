...
| Tier | When | What |
| --- | --- | --- |
| Inner loop | while editing | `dev.sh test --only Class`, `dev.sh py` (changed consumers by default; `--all`, `--lane`, or explicit modules), `dev.sh lint`. Quarantine (`config/test-quarantine.json`) applies only under `VOCELLO_QUARANTINE=1`, which CI sets; local runs execute quarantined tests. `dev.sh check` compiles the XCUITest bundles (`scripts/build_ui_test_bundles.sh`) when the dirty tree touches `Tests/*UITests`, `Tests/UIAutomationSupport` or `project.yml` — build only, never a run. CI still never compiles them: `repo_invariants.sh` check #2 forbids naming those schemes there, and that invariant is about executing XCUITest, not compiling it. |
