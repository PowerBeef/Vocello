...
- **`DP-28`** (planned) — Calibrate the layered local delivery evaluator.
  gate: V1 remains valid with full SER posteriors, entropy/margin abstention, sequential acoustic/ASR/identity/UTMOS/SER composition, and ridge-v1. The versioned v2 foundation extends the same source-bound harness: an atomic content-addressed cache binds original/canonical audio to analyzer/model/preprocessing provenance; a bounded five-region contour layer emits same-identity instructed-minus-neutral deltas; v2 listener packets add independent order, 12.5% repeats, anchors, uncertainty/confidence/replay/latency, target-neutral and candidate-baseline 2AFC, naturalness/intensity, retained listener rows, pairwise CCC, free-identification agreement, order bias and fluent coverage; v2 evaluation adds preset-local regularized logistic heads, elastic-net/PLS challengers, fold-local speaker/translation/seed/language validation, blocked conformal intervals, robust Mahalanobis/nearest-neighbor OOD and typed contradictions/abstention. A serial supervisor and cascade require generator exit before heavy analyzers and explain rejection, abstention and manual-listening routes. SenseVoiceSmall Q8 and DistilHuBERT are unadopted contract candidates; no weights were acquired. The first untracked 27-clip English v1 packet remains deliberately unqualified. Closure still requires AV-07's frozen six-speaker/three-script/eight-preset blinded labels, anchors and qualified listeners, independent untouched multi-speaker/script/language holdout, two clean canonical M2/8 GB heavy-adapter runs with stable post-exit memory, challenger license/training/digest provenance, and demonstrated untouched-holdout calibration gain over ridge-v1 without any VAD or preset regression. Automatic layers cannot authorize semantic promotion and production copy remains unchanged.
