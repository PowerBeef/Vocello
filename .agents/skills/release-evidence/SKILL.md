...
`docs/reference/agent-rules/release.md`. Candidate production and public promotion are separate; neither is
...
1. Require a tag as the argument. `git tag -l <requested-tag> -n1` and `git log -1 <requested-tag>` to confirm it exists and
...
3. `python3 scripts/check_release_notes.py <requested-tag>` for curated notes.
...
5. `gh run list --limit 10` for the release workflows and `gh release view <requested-tag>` for draft state.
