...
- Regenerate the charts (`python3 scripts/generate_readme_charts.py`; its `--check`
  ...
  records that share its comparison key; a lineage change resets the pool.
- Update `website/src/sections/Engineering.jsx`: the `MODES` medians (the generator
  prints them), the aria-label ranges, and the provenance record ID (the anchor).
