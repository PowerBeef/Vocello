...
final class TTSEngineStore: ObservableObject, TTSEngine {
    ...
    /// Imperative bridge for shells that must react to engine state without
    /// observing this whole object in a view body (the macOS root shell
    /// subscribes with `onReceive`; W1-D/W2-A). Fires once per applied
    /// frontend-state change, after the published properties are updated.
    let snapshotUpdates = PassthroughSubject<TTSEngineFrontendState, Never>()
    ...
    private func syncFromSnapshot(_ snapshot: TTSEngineFrontendState) {
        ...
        let frontendStateChanged = frontendState != retainedSnapshot
