...
description: Run one headless (non-UI) physical-iPhone diagnostic verb through scripts/ios_device.sh (doctor, device-state, crashes, bench, lang-bench, memory, profile, gate, or any other verb its usage lists). User-invoked only.
argument-hint: "<verb from scripts/ios_device.sh help> [options]"
...
2. Require a verb as the first argument; `scripts/ios_device.sh help` lists every verb. Never use `install` or `launch` against a distribution
